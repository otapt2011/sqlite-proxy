...
// Read the WASM file from the 'wasm' folder at the project root
const wasmPath = path.join(process.cwd(), 'wasm', 'sql-wasm.wasm');
...
export default async function handler(req, res) {
  ...
  try {
    // Initialize SQL.js with the local WASM binary
    const SQL = await initSqlJs({ wasmBinary });

    // Load database from Redis or file
    let dbData = null;
    let redis = null;
    try {
      redis = Redis.fromEnv();
